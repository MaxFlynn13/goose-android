...
class CloudApiClient(private val settingsStore: SettingsStore) {
    ...
    suspend fun callCloudApiStreaming(
        ...
        Log.i(TAG, "callCloudApiStreaming: provider=$provider, model=$resolvedModel, messageCount=${messages.size}")

        try {
            when (provider) {
                "anthropic" -> {
                    Log.i(TAG, "callCloudApiStreaming: routing to streamAnthropic, url=https://api.anthropic.com/v1/messages")
                    streamAnthropic(apiKey, resolvedModel, messages, activeSystemPrompt, imageAttachments, callbacks)
                }
                "openai" -> {
                    val url = "https://api.openai.com/v1/chat/completions"
                    Log.i(TAG, "callCloudApiStreaming: routing to streamOpenAI, url=$url")
                    streamOpenAI(apiKey, resolvedModel, messages, activeSystemPrompt,
                        url, emptyMap(), imageAttachments, callbacks)
                }
                "google" -> {
                    Log.i(TAG, "callCloudApiStreaming: routing to streamGoogle, model=$resolvedModel")
                    streamGoogle(apiKey, resolvedModel, messages, activeSystemPrompt, imageAttachments, callbacks)
                }
                "mistral" -> {
                    val url = "https://api.mistral.ai/v1/chat/completions"
                    Log.i(TAG, "callCloudApiStreaming: routing to streamOpenAI (Mistral), url=$url")
                    streamOpenAI(apiKey, resolvedModel, messages, activeSystemPrompt,
                        url, emptyMap(), imageAttachments, callbacks)
                }
                "openrouter" -> {
                    val url = "https://openrouter.ai/api/v1/chat/completions"
                    Log.i(TAG, "callCloudApiStreaming: routing to streamOpenAI (OpenRouter), url=$url")
                    streamOpenAI(apiKey, resolvedModel, messages, activeSystemPrompt,
                        url,
                ...
                "custom" -> {
                    ...
                    Log.i(TAG, "callCloudApiStreaming: routing to streamOpenAI (Custom), url=$url, customModel=$customModel")
                    streamOpenAI(apiKey, customModel.ifBlank { resolvedModel }, messages, activeSystemPrompt,
                        url, emptyMap(), imageAttachments, callbacks)
                }
                else -> {
                    val errorMsg = "Unknown provider: $provider"
                    Log.e(TAG, "callCloudApiStreaming: $errorMsg")
                    callbacks.onError(errorMsg)
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Streaming API call failed ($provider)", e)
            val errorMsg = "API Error ($provider): ${e.message}"
            callbacks.onError(errorMsg)
        }
    }
    ...
    private fun executeWithRetry(request: Request): okhttp3.Response {
        ...
        for (attempt in 0..MAX_RETRIES) {
            ...
            try {
                ...
                Log.i(TAG, "executeWithRetry: HTTP ${response.code} for ${request.url}")
    ...
    suspend fun callCloudApiForSummary(
        ...
        val resolvedModel = model.ifBlank { getDefaultModel(provider) }
        Log.i(TAG, "callCloudApiForSummary: provider=$provider, model=$resolvedModel")
    ...
    private suspend fun streamAnthropic(
        ...
        try {
            ...
            val response = executeWithRetry(request)
            Log.i(TAG, "streamAnthropic: HTTP ${response.code}")
            response.use { resp ->
                if (!resp.isSuccessful) {
                    val error = resp.body?.string() ?: "HTTP ${resp.code}"
                    val errorMsg = "Anthropic API error (${resp.code}): $error"
                    Log.e(TAG, "streamAnthropic: $errorMsg")
                    callbacks.onError(errorMsg)
                    return@withContext
                }
                        ...
                        try {
                            ...
                            when (type) {
                                ...
                                "error" -> {
                                    ...
                                    Log.e(TAG, "streamAnthropic: stream error event: $errorMsg")
                                    callbacks.onError("Stream error: $errorMsg")
                                    return@withContext
                                }
                            }
            ...
            Log.e(TAG, "streamAnthropic: exception", e)
            callbacks.onError("Anthropic streaming error: ${e.message}")
        }
    }
    ...
    private suspend fun streamOpenAI(
        ...
        try {
            ...
            val response = executeWithRetry(requestBuilder.build())
            Log.i(TAG, "streamOpenAI: HTTP ${response.code} for $endpoint")
            response.use { resp ->
                if (!resp.isSuccessful) {
                    val error = resp.body?.string() ?: "HTTP ${resp.code}"
                    val errorMsg = "API error (${resp.code}): $error"
                    Log.e(TAG, "streamOpenAI: $errorMsg")
                    callbacks.onError(errorMsg)
                    return@withContext
                }
            ...
            Log.e(TAG, "streamOpenAI: exception for $endpoint", e)
            callbacks.onError("OpenAI-compatible streaming error: ${e.message}")
        }
    }
     ...
     * API key is passed as a header (x-goog-api-key), NOT as a query parameter.
     */
    private suspend fun streamGoogle(
        ...
        try {
            ...
            // API key passed as header (x-goog-api-key), NOT as a query parameter
            val url = "https://generativelanguage.googleapis.com/v1beta/models/$model:streamGenerateContent?alt=sse"
            Log.i(TAG, "streamGoogle: url=$url, model=$model")
            ...
            val response = executeWithRetry(request)
            Log.i(TAG, "streamGoogle: HTTP ${response.code}")
            response.use { resp ->
                if (!resp.isSuccessful) {
                    val error = resp.body?.string() ?: "HTTP ${resp.code}"
                    val errorMsg = "Google AI error (${resp.code}): $error"
                    Log.e(TAG, "streamGoogle: $errorMsg")
                    callbacks.onError(errorMsg)
                    return@withContext
                }
            ...
            Log.e(TAG, "streamGoogle: exception", e)
            callbacks.onError("Google AI streaming error: ${e.message}")
        }
    }
}
